...
df_sowfa["sim_case"] = df_sowfa.apply(
    lambda x: "case_" + str(int(np.floor(x.d_spacing)) + 1), axis=1
)
...
# df_norm.head()
df_norm["power"] = (
    df_norm["power"] / df_norm.groupby("sim_case").transform(np.min)["power"]
)
df_norm["default"] = (
    df_norm["default"] / df_norm.groupby("sim_case").transform(np.min)["default"]
)
df_norm["merge"] = (
    df_norm["merge"] / df_norm.groupby("sim_case").transform(np.min)["merge"]
)
df_norm["legacy"] = (
    df_norm["legacy"] / df_norm.groupby("sim_case").transform(np.min)["legacy"]
)
