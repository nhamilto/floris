...
ws = 9.0
ccb.get_steady_state(
    turbine_dict_base, rotor_base, ws, title="Baseline", show_plot=True
)
ccb.get_steady_state(
    turbine_dict_scaled_2mw, rotor_scaled_2mw, ws, title="Scaled", show_plot=True
)


# Now get the full curves
ws_array = np.arange(5, 25, 2.0)
(
    ws_array,
    pow_array_base,
    cp_array_base,
    ct_array_base,
) = ccb.get_wind_sweep_steady_values(turbine_dict_base, rotor_base, ws_array=ws_array)
(
    ws_array,
    pow_array_scaled,
    cp_array_scaled,
    ct_array_scaled,
) = ccb.get_wind_sweep_steady_values(
    turbine_dict_scaled_2mw, rotor_scaled_2mw, ws_array=ws_array
)
